...
const MapScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchSupermarkets = async (latitude, longitude) => {
      try {
        const response = await fetch(
          `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=supermarket&key=${GOOGLE_PLACES_API_KEY}`
        );
    ...
    const getLocation = async () => {
      ...
      setRegion({
        latitude,
        longitude,
        latitudeDelta: calculateDelta(latitude, 150),
        longitudeDelta: calculateDelta(longitude, 150),
      });
  ...
  const handleMarkerPress = async (supermarket) => {
    ...
    if (mapRef && mapRef.current) {
      ...
      try {
        const response = await fetch(
          `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=1500&type=supermarket&key=${GOOGLE_PLACES_API_KEY}`
        );
        const data = await response.json();
        if (data.status === "OK") {
          setSupermarkets(data.results);
    ...
    <View style={{ flex: 1 }}>
          ...
          {supermarkets.map((supermarket) => (
            ...
            >
              ...
              <Callout style={styles.callout} anchor={{ x: 0.5, y: 1 }}>
                ...
                <Button
                  title="Check Out Items Here"
                  onPress={() => navigateToSearch(supermarket.name)}
                />
              </Callout>
            </Marker>
          ))}
      ...
      <View style={styles.navigationButtonsContainer}>
        ...
        <View style={styles.horizontalNavigationButtonsContainer}>
          <TouchableOpacity
            style={styles.horizontalNavigationButton}
